 ...
 * (from /api/dashboard) so the chart and the gauge agree.
 * ------------------------------------------------------------------------- */
...
export function buildSeries(
  ...
  const amp = clamp(18 + conviction * 0.18, 12, 34);
  ...
  for (let i = 0; i < range.points; i++) {
    ...
    out.push({ t, price: +price.toFixed(2), conviction: Math.round(value) });
  }
