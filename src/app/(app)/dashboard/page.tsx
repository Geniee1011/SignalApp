  ...
  return (
    <div>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        {markets.map((m) => (
          <InstrumentCard key={m.market} m={m} selected={sel?.market === m.market} onClick={() => setSelected(m.market)} />
        ))}
        {markets.length === 0 && (
          <Card className="col-span-full p-8 text-center text-sm text-muted">
            {loading ? "Loading markets…" : "No markets available for your access."}
          </Card>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_320px]">
        <Card className="p-4">
          <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
            <div>
              <span className="text-sm font-semibold">{sel ? `${sel.market} · Live Range` : "Live Range"}</span>
              {sel && <span className="ml-2 text-xs text-muted">{sel.name} · {sel.exchange}</span>}
            </div>
...
function InstrumentCard({ m, selected, onClick }: { m: DashboardMarket; selected: boolean; onClick: () => void }) {
  const high = m.conviction >= 55;
  return (
    <button
      onClick={onClick}
      className={cn(
        "rounded-[var(--radius-card)] border bg-surface p-3 text-left transition",
    ...
    >
      ...
      <div className="mt-3 flex items-baseline justify-between">
        <span className="text-[10px] uppercase tracking-wide text-muted">Conviction</span>
        <span className={cn("text-lg font-semibold nums", high ? "text-primary" : "text-foreground")}>{m.conviction}%</span>
      </div>
      <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-surface-3">
        <div className="h-full rounded-full transition-all" style={{ width: `${m.conviction}%`, background: high ? BLUE : "var(--color-muted-2, #55637a)" }} />
      </div>
      <div className="mt-2 text-[11px] text-muted-2">Bias: <span className="text-muted">{m.bias ?? "—"}</span></div>
    </button>
  );
}

function StatusBadge({ status }: { status: DashboardMarket["status"] }) {
  if (status === "active")
    return <span className="rounded-md bg-primary/20 px-1.5 py-0.5 text-[10px] font-semibold text-primary">● Active Trade</span>;
  if (status === "in_play")
    return <span className="inline-flex items-center gap-1 rounded-md border border-border bg-surface-2 px-1.5 py-0.5 text-[10px] font-medium text-muted"><span className="h-1.5 w-1.5 rounded-full bg-long" />In Play</span>;
  return <span className="inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[10px] font-medium text-muted-2"><span className="h-1.5 w-1.5 rounded-full bg-muted-2" />Standby</span>;
}
...
function LiveRangeChart({ series }: { series: SeriesPoint[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: "block", height: "auto" }}>
      ...
      {/* horizontal gridlines + right (conviction) axis */}
      {convTicks.map((c) => (
        <g key={c}>
